...
router.get(`/`, async (req, res) => {
  try {
    ...
    if (!orderItemList || orderItemList.length === 0) {
      return res
        .status(404)
        .json({ success: false, message: "No orderItems found" });
    }
      ...
      return {
        ...
        product: {
          ...
          images: product.images.map(
            (image) => `/tttn/product//gallery/${product.id}/images/${image.id}`
          ),
          price: product.price,
...
router.get("/:id", async (req, res) => {
  try {
    ...
    if (!orderItem) {
      res
        .status(404)
        .json({ message: "The orderItem with the given ID was not found." });
    } else {
  ...
  try {
    ...
    if (existingOrderItem) {
      ...
      if (existingUser) {
        ...
        const isProductUserMatch = formattedOrderItems.some(
          (item) =>
            item.product.user.id === product.user.id &&
        ...
        if (isProductUserMatch) {
          ...
          if (!orderItem) {
            return res.status(400).json({
              success: false,
              error: "The orderItem cannot be created!",
            });
          }
  ...
  OrderItem.findByIdAndRemove(req.params.id)
    .then((orderItem) => {
      if (orderItem) {
        return res
          .status(200)
          .json({ success: true, message: "the orderItem is deleted!" });
      } else {
        return res
          .status(404)
    ...
    .then((result) => {
      if (result.deletedCount > 0) {
        return res.status(200).json({
          success: true,
          message: `All orderItems for user ${userId} are deleted!`,
        });
      } else {
        return res.status(404).json({
          success: false,
          message: `No orderItems found for user ${userId}`,
        });
      }
    })
    .catch((err) => {
